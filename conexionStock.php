<?php
	...
	if(isset($_POST['nombreRepuesto'])) {
		...
		if(empty($nombreRepuesto) || empty($rutProveedor)){
			...
			$consulta="SELECT rutProveedor FROM proveedor WHERE rutProveedor = '$rutProveedor'";
			$res=mysqli_query($conexion, $consulta) or die ("Error en la consulta del proveedor");
			...
			if (count($vacio)>0 || count($err)>0){
				...
				//Si existe Proveedor solo ascociar repuesto al proveedor ya existente
				if (empty($numero)){
					$consulta="INSERT INTO Proveedor (rutProveedor, nombreProveedor, direccionSucursalCercana) VALUES ('$rutProveedor', '$nombreProveedor', '$direccionSucursalCercana')";
					mysqli_query($conexion, $consulta) or die ("Error:"."<br>"."Error al subir los datos proveedor");
					$consulta="INSERT INTO telefonospoveedor (rutProveedor,telefonosProveedor) VALUES ('$rutProveedor', '$telefonoProveedor')";
					mysqli_query($conexion, $consulta) or die ("Error:"."<br>"."Error al subir telefono");
				}
				$consulta="INSERT INTO repuesto(nombreRepuesto, marcaRepuesto, desRepuesto, cantidadRepuesto, precioRepuesto, urlImagen, rutProveedor, recordatorio) VALUES ('$nombreRepuesto', '$marcaRepuesto','$desRepuesto', '$cantidadRepuesto', '$precioRepuesto', '$new_img_name','$rutProveedor', '$limiteRepuesto')";
